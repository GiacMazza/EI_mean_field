program officina
  ...
  real(8) :: Evalence,Econduction,tconduction,tvalence,tt_hyb
  real(8) :: w0gap
  ...
  do ik=1,Lk
     do ispin=1,Nspin
        ...
        Rlat=R1
        Hk_w90(iso,jso,ik) = tt_hyb
        Hk_w90(jso,iso,ik) = conjg(Hk_w90(iso,jso,ik)) !hybloc*(1.d0-exp(-xi*dot_product(Rlat,kpt_latt(ik,:))))
        !
        iorb=1
        jorb=3
        iso=(ispin-1)*Norb+iorb
  ...
  do ihf=1,Nhf
     ...
     x_iter_=x_iter
     !
     H_Hf=HF_hamiltonian_bls(x_iter)
     H_Hf=H_Hf+Hk_w90
     !
     call fix_mu(H_Hf,delta_hf,mu_fix)
     !
  ...
  do i=1,2
     ...
     do ik=1,100
        ...
        ! if(j.eq.1) then
        !    do iso=1,Nso
        !       write(905,*) Hktmp(iso,1),Hktmp(iso,2),ek_out(1),ek_out(2)
        !    end do
        ! end if
        !
...
contains
  ...
  function root_find_HF(x) result(out_x)
    ...
    if(size(x).ne.11) stop 'delta HF s(x)/= 18'
    !
    do i=1,11
       xhf(i) = x(i) + xi*0.d0
    end do
    ...
    xhf_(9) = delta_hfr(3,3,ir0+1)
    !
    xhf_(10) = delta_hfr(1,3,ir0) + delta_hfr(1,3,ir0+1)
    xhf_(11) = delta_hfr(2,3,ir0) + delta_hfr(2,3,ir0+1)
    !
    xhf_=xhf_-xhf
    do i=1,11
       out_x(i)=dreal(xhf_(i))
  ...
  function HF_hamiltonian_bls(x_iter) result(Hhf)
    implicit none
    complex(8) :: x_iter(11)
    complex(8),dimension(Nso,Nso,Lk) :: Hhf
    integer :: iso,jso,iorb,jorb    
    ...
    do ik=1,Lk       
       ...
       do ispin=1,Nspin
          ...
          Hhf(jso,iso,ik) = -Vcell*x_iter(4)*(1.d0-exp(-xi*dot_product(R1,kpt_latt(ik,:))))
          Hhf(jso,iso,ik) = Hhf(jso,iso,ik) - Vcell*x_iter(10)*exp(-xi*dot_product(R1,kpt_latt(ik,:)))
          if(modk.lt.1.d-10) Hhf(jso,iso,ik) = Hhf(jso,iso,ik) + cf_ext
          Hhf(iso,jso,ik) = conjg(Hhf(jso,iso,ik))
          !
          iorb=2; iso=(ispin-1)*Norb+iorb          
          jorb=3; jso=(ispin-1)*Norb+jorb
    ...
    !
  end function HF_hamiltonian_bls
