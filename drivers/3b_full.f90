program officina
  ...
  !
  x_iter(7) = delta_hfr(1,1,ir0+1)
  x_iter(8) = delta_hfr(2,2,ir0+1)
  x_iter(9) = delta_hfr(3,3,ir0+1)
  !  
  x_iter(10) = delta_hfr(1,3,ir0)+delta_hfr(1,3,ir0+1)
  x_iter(11) = delta_hfr(2,3,ir0)+delta_hfr(2,3,ir0+1)
  !
  ...
  if(fix_phi) then
     uio=free_unit()
     open(unit=uio,file='loop_fixed_phi.out')
     ! unit_in=free_unit()
     ...
     do ihf=1,50
        ...
        do jhf=1,Nhf_
           ...
           err=0.d0
           do i=1,9
              err=err+abs(x_iter(i)-x_iter_(i))
           end do
           if(err.lt.1.d-6) exit 
           !
        end do
        write(489,'(10F18.10)') 
        write(489,'(10F18.10)') 
        ...
        call fsolve(root_find_inner_loop_free,xr_tmp,tol=1.d-12)
        do i=1,9
     ...
     uio=free_unit()
     open(unit=uio,file='loop_fixed_n.out')

     xphi=-0.05d0
     do ihf=1,20
        !
        xphi=xphi+0.05d0
        write(*,*) 'fixed n1/n2 loop',ihf,xphi
        xpi(1) =   0.5d0 !xphi
        xpi(2) =  -0.5d0!-xphi
        !
        x_iter(1) = xphi
        x_iter(2) = xphi
        x_iter(3) = 2.d0-2.d0*xphi
        !
        err=1.d0
        do jhf=1,Nhf_
           x_iter_=x_iter
           write(490,'(10F18.10)') err
           !
           H_Hf=HF_hamiltonian(x_iter)
           H_Hf=H_Hf+Hk_w90
           !
        ...
        !        
        !        
        write(uio, '(20F18.10)') dreal(xphi),dreal(delta_hfr(1,1,ir0)+delta_hfr(1+Norb,1+Norb,ir0)), &
             dreal(delta_hfr(2,2,ir0)+delta_hfr(2+Norb,2+Norb,ir0)), &
             dreal(delta_hfr(3,3,ir0)+delta_hfr(3+Norb,3+Norb,ir0)), &
             dreal(x_iter(4:11)),Eout+mu_fix*(dreal(x_iter(1))+dreal(x_iter(2))+dreal(x_iter(3))),Eout
  ...
  if(hf_in) then
     ...
     if(flen.eq.11) then
        ...
        xhf(1:3) = x_iter(1:3)
        xhf(4:5) = x_iter(10:11)
     end if
     !
  else

     xhf(1:2) = 0.d0; xhf(3) = 2.d0
     xhf(4) = 0.5d0 ; xhf(5) = -0.67    
     !
     x_iter(4) = 0.1d0
     x_iter(5) = 0.1d0
     !  
  ...
  do ihf=1,Nhf
     ...
     do jhf=1,Nhf_
        !
        if(jhf.eq.1) write(*,*) 'self-consistency phi loop',ihf,xpi
        x_iter_=x_iter
        !
        H_Hf=HF_hamiltonian(x_iter)
        H_Hf=H_Hf+Hk_w90
        ...
        !
        x_iter=x_iter*wmix+(1.d0-wmix)*x_iter_
        !write(678,'(20F18.10)') dreal(x_iter(:))        
        !
     end do
     ! write(678,'(20F18.10)')
     ! write(678,'(20F18.10)') 
     ...
     !
     xhf(1)=dreal(delta_hfr(1,1,ir0)+delta_hfr(4,4,ir0))
     xhf(2)=dreal(delta_hfr(2,2,ir0)+delta_hfr(5,5,ir0))
     xhf(3)=dreal(delta_hfr(3,3,ir0)+delta_hfr(6,6,ir0))
     !
     xhf(4)=dreal(delta_hfr(1,3,ir0)+delta_hfr(1,3,ir0+1))
     xhf(5)=dreal(delta_hfr(2,3,ir0)+delta_hfr(2,3,ir0+1))
     !
     xhf=wmix*xhf+(1-wmix)*xhf_
     !
     !+- double counting term -+!
     Eout=Eout-Ucell*0.25d0*(dreal(x_iter(1))**2.d0+dreal(x_iter(2))**2.d0+dreal(x_iter(3))**2.d0)
     Eout=Eout-2*Vcell*(dreal(x_iter(1))*dreal(x_iter(3))+dreal(x_iter(2))*dreal(x_iter(3)))
     ...
     Eout=Eout + 2.d0*Wcell*abs(x_iter(6))
     !
     ! Eout=Eout - Wcell*abs(x_iter(1))**2.d0 - Wcell*abs(x_iter(2))**2.d0-Wcell*abs(x_iter(3))**2.d0
     !
     ! Eout=Eout + 2.d0*Wcell*abs(x_iter(7))**2.d0 
     ! Eout=Eout + 2.d0*Wcell*abs(x_iter(8))**2.d0 
     ! Eout=Eout + 2.d0*Wcell*abs(x_iter(9))**2.d0      
     !
  ...
  !
  do ihf=1,1 !+- qui di fatto ne serve solo uno di loop HF
     !
     xphi_=xphi
     do jhf=1,Nhf_
        !
        if(jhf.eq.1) write(*,*) 'self-consistency phi loop',ihf,xphi
        x_iter_=x_iter
        !
        H_Hf=HF_hamiltonian(x_iter)
        H_Hf=H_Hf+Hk_w90
     ...
     ! Eout=Eout + 2.d0*Wcell*abs(x_iter(9))**2.d0      
     !
     write(uio, '(20F18.10)') dreal(xphi),dreal(xphi),dreal(x_iter(1:9)),Eout+mu_fix*(dreal(x_iter(1))+dreal(x_iter(2))+dreal(x_iter(3))), &
          dreal(delta_hfr(1,3,ir0)+delta_hfr(1,3,ir0+1)),dreal(delta_hfr(2,3,ir0)+delta_hfr(2,3,ir0+1))
     !
     !
  end do
...
contains
  ...
  !   !
  !   xhf_(7) = delta_hfr(1,1,ir0+1)
  !   xhf_(8) = delta_hfr(2,2,ir0+1)
  ...
  function HF_hamiltonian(x_iter) result(Hhf)
    ...
    do ik=1,Lk
       do ispin=1,Nspin
          ...
          ! Hhf(iso,iso,ik) = Hhf(iso,iso,ik) + 2.d0*Wcell*x_iter(1)
          ! Hhf(iso,iso,ik) = Hhf(iso,iso,ik) - 2.d0*Wcell*dreal((x_iter(7)*exp(-xi*dot_product(R1,kpt_latt(ik,:)))))
          !
          iorb=2
          iso=(ispin-1)*Norb+iorb
          Hhf(iso,iso,ik) = 0.5d0*Ucell*x_iter(2)+2.d0*Vcell*x_iter(3)          
          ! Hhf(iso,iso,ik) = Hhf(iso,iso,ik) + Wcell*x_iter(1) 
          ! Hhf(iso,iso,ik) = Hhf(iso,iso,ik) + 2.d0*Wcell*x_iter(2)
          ! Hhf(iso,iso,ik) = Hhf(iso,iso,ik) - 2.d0*Wcell*dreal((x_iter(8)*exp(-xi*dot_product(R1,kpt_latt(ik,:)))))
          !
          iorb=3
          iso=(ispin-1)*Norb+iorb
          Hhf(iso,iso,ik) = 0.5d0*Ucell*x_iter(3)+2.d0*Vcell*(x_iter(2)+x_iter(1))
          ! Hhf(iso,iso,ik) = Hhf(iso,iso,ik) + 2.d0*Wcell*x_iter(3)          
          ! Hhf(iso,iso,ik) = Hhf(iso,iso,ik) - 2.d0*Wcell*dreal((x_iter(9)*exp(-xi*dot_product(R1,kpt_latt(ik,:)))))
          !
          ...
          ! Hhf(jso,iso,ik) = Hhf(jso,iso,ik)-Wcell*x_iter(6)  !*(1.d0-exp(-xi*dot_product(R1,kpt_latt(ik,:))))
          ! Hhf(iso,jso,ik) = conjg(Hhf(jso,iso,ik))
          !
       end do
    end do
